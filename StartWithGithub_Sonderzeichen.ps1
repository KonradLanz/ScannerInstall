...
function Invoke-WingetInstall {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ⚠ Winget installation may have issues, but continuing..." -ForegroundColor Yellow
    }
...
if (-not $SkipInstall) {
    ...
    if (-not (Test-CommandExists "gh")) {
        ...
        Write-Host "  ✓ GitHub CLI bereits installiert" -ForegroundColor Green
    }
    ...
    if (Test-CommandExists "gh") {
        ...
        Write-Host "  ⚠ GitHub CLI konnte nicht installiert werden. Versuche trotzdem weiterzumachen..." -ForegroundColor Yellow
    }
} else {
...
if (Test-CommandExists "gh") {
    ...
    Write-Host "  ⚠ GitHub CLI nicht verfügbar - überspringe GitHub Auth" -ForegroundColor Yellow
}
...
if (-not (Test-Path $TargetDir)) {
    ...
    Write-Host "  ✓ Verzeichnis existiert: $TargetDir" -ForegroundColor Green
}
    ...
    if (-not $DryRun) {
        ...
        if (Test-CommandExists "gh") {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host "  ✓ Repository existiert bereits, pushe nur..." -ForegroundColor Green
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Host "  ⚠ Repository-Erstellung könnte fehlgeschlagen sein" -ForegroundColor Yellow
                }
            }
        } else {
            Write-Host "  ✗ GitHub CLI nicht verfügbar - kann nicht zu GitHub pushen" -ForegroundColor Red
        }
    } else {
