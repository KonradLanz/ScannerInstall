...
#  █ Compatible with PowerShell 5.1 and 7.5+                     █
...
try {
    ...
    if ($scriptPath) {
        Unblock-File -Path "$scriptPath\*.ps1" -ErrorAction SilentlyContinue
    }
} catch {
...
$Strings = @{
    ...
    de = @{
        ...
        script_failed      = "Script-Ausführung fehlgeschlagen"
    }
}
...
if ($success) {
    ...
    Write-Host ""
    Write-Host "✗ $($Text.script_failed)" -ForegroundColor Red
    Write-Host ""
    exit 1
}
